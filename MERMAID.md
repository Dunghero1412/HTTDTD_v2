...
```mermaid
flowchart LR
    subgraph GUI["🖥️ GUI Thread (PyQt6)"]
        A1[Nút bấm]
        A2[Hiển thị Log]
        A3[Hiển thị Bảng điểm]
    end

    subgraph CTRL["📡 Controller Thread"]
        B1[LoRa Gửi]
        B2[LoRa Nhận]
        B3[Score Manager]
    end
    ...
    subgraph QUEUE["📨 Hàng đợi"]
        C1[cmd_queue<br/>GUI → Controller]
        C2[out_queue<br/>Controller → GUI]
    end

    A1 -->|"Lệnh send/reset_round"| C1
    C1 -->|"Xử lý lệnh"| B1
    B1 -->|"UP/DOWN"| NODE[Node]
    NODE -->|"Tọa độ"| B2
    B2 -->|"Cập nhật"| B3
    B3 -->|"Log, Board"| C2
    C2 -->|"Hiển thị"| A2
